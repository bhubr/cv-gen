...
\definecolor{awesome}{HTML}{00796B} % Uncomment if you would like to specify your own color
...
\github{<%= github %>}
\linkedin{<%= linkedin %>}
<% if (typeof skype !== 'undefined') { %>%\skype{<%= skype %>}<% } %>
...
\makecvfooter{\today}{<%= firstname %> <%= lastname %>~~~·~~~Résumé}{\thepage} % Specify the letter footer with 3 arguments: (<left>, <center>, <right>), leave any of these blank if they are not needed
